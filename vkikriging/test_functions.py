...
Schwefel, Camel, GenzFunction1.	 Each has `f`, `df`, `ddf` function members for value, derivative and
...
class GenzFunction1(TestFunction):
	"""
	Genz functions, in d-dimensions. 
	"""
	
	def __init__(self, d=2):
		self.d = d
		self.xmin, self.xmax = np.zeros(d), np.ones(d)
		self.xopt = np.zeros(d)
		self.fopt = -1

	def f(self, xx):
		return np.cos(np.pi + np.sum(19./5 * xx))

	def df(self, xx):
		return -np.sin(np.pi + np.sum(19./5 * xx)) * np.ones(self.d) * 19./5
...
class NoisyParabola(TestFunction):
...
class Rosenbrock(TestFunction):
	...
	def __init__(self):
		self.d = 2
		self.xmin, self.xmax = np.array([-2, -1]), np.array([2, 3])
		self.xopt = np.array([1., 1.])
		self.fopt = 0.
...
class Camel(TestFunction):
	"""Six-hump camel-back test function in 2-d."""

	def __init__(self):
		self.d = 2
		self.xmin, self.xmax = np.array([-3, -2]), np.array([3, 2])
		self.xopt = np.array([-0.0898, 0.7126])	 # and (0.0898, -0.7126)
		self.fopt = -1.0316
